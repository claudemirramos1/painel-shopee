...
# Configurações de Redes Sociais obtidas via Secrets
...
def carregar_rascunhos():
    """Busca todas as ofertas salvas no banco de dados do Supabase."""
    try:
...
def remover_rascunho(rascunho_id):
    """Remove uma oferta do banco após o disparo ou descarte."""
    try:
        supabase.table("ofertas").delete().eq("id", rascunho_id).execute()
    except Exception as e:
        st.error(f"Erro ao remover oferta: {e}")
...
with aba_fila:
    ...
    if not rascunhos:
        ...
        for item in rascunhos:
            titulo_item = item.get('titulo') or 'Oferta sem Título'
            preco_item = item.get('preco') or '0,00'
            
            with st.expander(f"📦 {titulo_item} - R$ {preco_item}", expanded=False):
